...
import DESCRIPTION from "./webfetch.tool.txt"
...
class WebFetchTool extends BaseTool<typeof parameters> {
  ...
  protected override async run({ url, format = "markdown" }: z.infer<typeof parameters>) {
    ...
    try {
      ...
      const content = isHtml
        ? (format === "markdown" ? this.htmlToMarkdown(text) : this.htmlToText(text))
        : text
...
export default new WebFetchTool()
